        ...
        try {
            switch ($action) {
                ...
                case 'test_slack':
                    ...
                    if (!filter_var($webhookUrl, FILTER_VALIDATE_URL) || !isSupportedCommunityWebhookUrl($webhookUrl)) {
                        echo json_encode([
                            'success' => false,
                            'message' => 'Invalid webhook URL. Use a Slack incoming webhook (hooks.slack.com/services/…) or Discord webhook (discord.com/api/webhooks/…).'
                        ]);
                        exit;
                    }
                    ...
                    if (isDiscordWebhookUrl($webhookUrl)) {
                        $testMessage = [
                            'username' => 'ClaimIt',
                            'content' => '🧪 Test message from **ClaimIt** — your Discord webhook is configured correctly.',
                        ];
                    } else {
                        $testMessage = [
                    ...
                    try {
                        error_log("Webhook test - sending to: " . $webhookUrl);
                        error_log("Webhook test - message: " . json_encode($testMessage));

                        [$httpCode, $response, $curlError] = postWebhookJson($webhookUrl, $testMessage);

                        error_log("Webhook test - HTTP code: $httpCode, Response: '$response', cURL Error: '$curlError'");

                        $slackOk = $httpCode === 200 && trim(strtolower($response)) === 'ok';
                        $discordOk = $httpCode === 204 || ($httpCode >= 200 && $httpCode < 300);

                        if ($slackOk || $discordOk) {
                            $dest = isDiscordWebhookUrl($webhookUrl) ? 'Discord channel' : 'Slack channel';
                            echo json_encode(['success' => true, 'message' => "Test message sent successfully! Check your {$dest}."]);
                        } else {
                            $errorMsg = $curlError ?: "Webhook returned HTTP $httpCode with response: $response";
                            error_log("Webhook test failed: $errorMsg");
